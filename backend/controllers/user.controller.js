  ...
  try {
    ...
    if (!username || !email || !password) {
      return res.status(401).json({
        message: "Someting is missing, please check!",
        success: false,
      });
    }
...
export const followrOrUnfollow = async (req, res) => {
  try {
    ...
    // CHECK IF ITS TO FOLLOW OR UNFOLLOW
    const isFollowing = user.following.includes(toFollow);
    if (isFollowing) {
      // UNFOLLOW LOGIC
      await Promise.all([
        User.updateOne({ _id: follower }, { $pull: { following: toFollow } }),
        User.updateOne({ _id: toFollow }, { $pull: { followers: follower } }),
      ]);
      return res
        .status(200)
      ...
      // FOLLOW LOGIC
      await Promise.all([
        User.updateOne({ _id: follower }, { $push: { following: toFollow } }),
        User.updateOne({ _id: toFollow }, { $push: { followers: follower } }),
      ]);
      return res
        .status(200)
